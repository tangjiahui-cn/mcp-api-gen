...
除 MCP / CLI 外，仓库根目录内置 `skills/create-api` Skill，让支持 Agent Skills 的客户端（Claude Code、Cursor、Trae 等）无需配置 MCP，也能以 **Skill + CLI** 形式调用本项目完成生成。

### 1、Skills 形式（安装 Skill）

把 `skills/create-api` 目录安装到客户端 skills 目录即可，例如：
